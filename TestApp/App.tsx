...
function App() {
  ...
  const flipperConnection = useRef<Flipper.FlipperConnection | null>(null)
  ...
  const handleAddTask = useCallback(
    (description: string): void => {
      ...
      flipperConnection?.current?.send('newRow', task);
    },
    [realmRef],
  );
  ...
  addPlugin({
    ...
    onConnect(connection) {
      ...
      connection.send('newRow', { id: 1, title: 'test', url: 'whatever' });
      // connection.send('newRow', {id: 2, title: 'test', url: 'whatever'});
    },
    onDisconnect() {
